...
type
  TF_Renovacao = class(TForm)
    ...
    procedure Btn_RenovarEmprestimoClick(Sender: TObject);
...
procedure TF_Renovacao.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
  F_Renovacao := nil;
end;

procedure TF_Renovacao.MostraCampos;
begin
  ...
  Btn_RenovarEmprestimo.Visible := True;
  L_Encontrado.Visible := False;
end;

procedure TF_Renovacao.VerificaDevolucao;
var
  DiasAtraso : Double;
begin
  try
...
function TF_Renovacao.ConsultaCampo(pCampo: String; pTabela: String; pCondicao: String): String;
begin
  try
    ADOQ_Aux.Close;
    ADOQ_Aux.SQL.Clear;
...
procedure TF_Renovacao.EscondeCampos;
begin
  ...
  Btn_RenovarEmprestimo.Visible := False;
  E_CodLivro.Clear;
end;

procedure TF_Renovacao.SomenteNumeros(Sender: TObject; var Key: Char);
begin
  if not (Key in['0'..'9',Chr(8)]) then
    Key:= #0;
end;

procedure TF_Renovacao.E_CodLivroExit(Sender: TObject);
begin
  if not(E_CodLivro.Text = '') then
    VerificaDevolucao;
end;

procedure TF_Renovacao.FormKeyPress(Sender: TObject; var Key: Char);
begin
  F_Principal.TabEnter(Sender, Key);
end;

procedure TF_Renovacao.Btn_PesquisarClick(Sender: TObject);
begin
...
procedure TF_Renovacao.Btn_RenovarEmprestimoClick(Sender: TObject);
begin
  try
    ADOQ_Aux.Close;
    ADOQ_Aux.SQL.Clear;
    ADOQ_Aux.SQL.Add('update movitem');
    ADOQ_Aux.SQL.Add('   set data_devolucao=#' + FormatDateTime('mm/dd/yyyy', Date + 7) + '#');
    ...
    ADOQ_Aux.ExecSQL;
    MessageDlg('Renovação realizada com sucesso para o dia ' + FormatDateTime('dd/mm/yyyy', Date + 7) + ' .', mtInformation, [mbOK], 0);
  finally
    EscondeCampos;
    L_Encontrado.Visible := False;
  end;
